...
<div id="wrap_chat">
    ...
    <div id="wrap_chat_list" >
        <ul class="chat_list" id="chat_list">
            <template v-for="(_, chat) in chatList" >
                <li class="chat_timestamp" v-if="chat.chkFirstMsg === 'Y'" v-bind:key="_">{{moment(chat.createdTime).format('ll')}}</li>
                <li class="message" v-bind:key="_+chat.id" v-bind:class="chat.chkMyContent == 'N' ? 'incoming_message' : 'sent_message'">
